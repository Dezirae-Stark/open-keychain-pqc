# OpenKeychain PQC v6.0.4-pqc.2

Fixes an install-breaking bug in v6.0.4-pqc.1 — **use this release, not
v6.0.4-pqc.1.**

## Fix: package identity collision with classic OpenKeychain

v6.0.4-pqc.1 kept the same Android package name as classic upstream
OpenKeychain (`org.sufficientlysecure.keychain`). Android refuses to install
an APK "update" over an existing app unless it's signed with the same key —
this release is necessarily signed with a new, dedicated key (we don't have
upstream's private signing key), so v6.0.4-pqc.1 would fail to install for
anyone who already had classic OpenKeychain on their device.

Fixed by renaming the package to `org.sufficientlysecure.keychain.pqc` and
the app label to "OpenKeychain PQC" — this release now installs as a
**separate app alongside** classic OpenKeychain rather than colliding with
it. No data loss, no need to uninstall anything.

Verified: full test suite green (357 tests), fresh install and launch
confirmed crash-free on a real device under the new package identity.

---

## What's new (unchanged from v6.0.4-pqc.1)
...
upstream OpenKeychain signing key, and installs under its own package
identity (`org.sufficientlysecure.keychain.pqc`) — separate from, and
side-by-side installable with, classic OpenKeychain.
