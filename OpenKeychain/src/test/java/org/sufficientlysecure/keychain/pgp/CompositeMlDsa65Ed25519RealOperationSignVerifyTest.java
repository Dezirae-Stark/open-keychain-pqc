...
/**
 * End-to-end round trip for composite ML-DSA-65+Ed25519 (algorithm ID 30) OpenPGP signing,
 ...
 * CompositeMlKem768X25519RealOperationEncryptDecryptTest} for the composite encryption phase.
 * <p>
 * The composite ML-DSA-65+Ed25519 key here is the keyring's <em>master</em> key (both
 * certifying and signing), matching the shape of the algorithm's own official test vector
 * (draft-ietf-openpgp-pqc-17's {@code v6-mldsa-65-sample-sk.asc}: a v6 ML-DSA-65+Ed25519
 ...
 * this algorithm, and a v6 master key's own direct-key/user-ID self-certifications must
 * therefore also be v6 composite signatures -- so key generation alone already exercises the
 * v6-signature-generator wiring ({@code CanonicalizedSecretKey#newSignatureGenerator},
 * {@code PgpKeyOperation#getSignatureGenerator}) before any message is ever signed.
 */
@RunWith(KeychainTestRunner.class)
public class CompositeMlDsa65Ed25519RealOperationSignVerifyTest {
    ...
    public static void setUpOnce() throws Exception {
        ...
        builder.addUserId("pqc-composite-sign-real-op-test <pqc-sign-real-op@example.org>");
        ...
        assertTrue("composite ML-DSA-65+Ed25519 key generation must succeed (this already "
     ...
     * was done separately, see the design doc / session notes for the KAT results).
     */
    @Test
    public void testRealSignVerifyOperationRoundTrip() throws Exception {
        ...
        String plaintext = "the OpenKeychain composite ML-DSA-65+Ed25519 round trip works end-to-end ☭";
        byte[] signedMessage;
        ...
        { // sign via the real PgpSignEncryptOperation, using the composite master key
            ...
            assertTrue("real PgpSignEncryptOperation.execute() must succeed signing with the "
                    + "composite ML-DSA-65+Ed25519 key: " + result.getLog(), result.success());
        ...
        { // negative control: a tampered signed message must be rejected, not silently accepted
            ...
            // the trailing signature packet's algorithm-specific (Ed25519||ML-DSA) data --
