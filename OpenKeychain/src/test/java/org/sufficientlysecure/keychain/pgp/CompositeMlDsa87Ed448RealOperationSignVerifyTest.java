...
/**
 * End-to-end round trip for composite ML-DSA-87+Ed448 (algorithm ID 31) OpenPGP signing,
 * exercised entirely through the same real, top-level app operation classes a caller of the
 * app actually uses -- {@link PgpSignEncryptOperation#execute} and {@link
 ...
 * CompositeMlDsa65Ed25519RealOperationSignVerifyTest} for the MUST-implement composite
 * signing algorithm.
 * <p>
 * The composite ML-DSA-87+Ed448 key here is the keyring's <em>master</em> key (both
 * certifying and signing), matching the shape of the algorithm's own official test vector
 * (draft-ietf-openpgp-pqc-17's {@code v6-mldsa-87-sample-sk.asc}: a v6 ML-DSA-87+Ed448
 * Private Key packet is the primary key, with a self-signed user ID). This is a structural
 * necessity, not a test-convenience choice: the draft mandates v6-only keys and signatures for
 * this algorithm too (no v4 allowance, same as algorithm 30), so a v6 master key's own
 * direct-key/user-ID self-certifications must therefore also be v6 composite signatures -- so
 * key generation alone already exercises the v6-signature-generator wiring ({@code
 * CanonicalizedSecretKey#newSignatureGenerator}, {@code PgpKeyOperation#getSignatureGenerator})
 * before any message is ever signed.
 */
@RunWith(KeychainTestRunner.class)
public class CompositeMlDsa87Ed448RealOperationSignVerifyTest {
    ...
    public static void setUpOnce() throws Exception {
        ...
        builder.addUserId("pqc-composite-sign-real-op-test-87 <pqc-sign-real-op-87@example.org>");
        ...
        assertTrue("composite ML-DSA-87+Ed448 key generation must succeed (this already "
     ...
     * rejected by the same real verification call path -- not just by the crypto core in
     * isolation (that cross-check against the official draft-ietf-openpgp-pqc-17 test vector
     * was done separately in {@code CompositeMlDsa87Ed448SignVerifyTest}).
     */
    @Test
    public void testRealSignVerifyOperationRoundTrip() throws Exception {
        ...
        String plaintext = "the OpenKeychain composite ML-DSA-87+Ed448 round trip works end-to-end ☭";
        byte[] signedMessage;

        { // sign via the real PgpSignEncryptOperation, using the composite master key
            ...
            assertTrue("real PgpSignEncryptOperation.execute() must succeed signing with the "
                    + "composite ML-DSA-87+Ed448 key: " + result.getLog(), result.success());
        ...
        { // verify via the real PgpDecryptVerifyOperation
            ...
            assertTrue("real PgpDecryptVerifyOperation.execute() must succeed verifying the "
                    + "composite-key-signed message: " + result.getLog(), result.success());
        ...
        { // negative control: a tampered signed message must be rejected, not silently accepted
            ...
            // the trailing signature packet's algorithm-specific (Ed448||ML-DSA) data --
            // exactly analogous to the crypto-core tamper checks, but through the real
            // verification call path this time.
            int flipOffset = tampered.length - 200;
            tampered[flipOffset] ^= 0x01;

            DecryptVerifyResult result = runVerify(tampered, null);

            Assert.assertEquals("a bit-flipped composite signature must be reported invalid, "
