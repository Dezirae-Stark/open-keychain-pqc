...
/**
 * Tests for standalone (non-composite, closed-ecosystem) ML-DSA-65 OpenPGP signing
 ...
 * end-to-end app-operation-level test ({@code StandaloneMlDsa65RealOperationSignVerifyTest})
...
public class StandaloneMlDsa65SignVerifyTest {
    ...
    public static void setUpOnce() throws Exception {
        ...
        // This codebase's own decision mandates v6-only keys/signatures for algorithm 102, and
        ...
        builder.addUserId("pqc-standalone-mldsa65-sign-test <pqc-standalone-sign@example.org>");
    ...
    public void testCanonicalizationKeepsStandaloneKey() {
        ...
        assertEquals(PublicKeyAlgorithmTags.EXPERIMENTAL_3, masterKey.getAlgorithm());
        assertTrue(masterKey.isStandaloneMlDsa65());
        assertTrue(masterKey.canSign());
        assertTrue(masterKey.canCertify());
        assertTrue("PgpSecurityConstants must not flag algorithm 102 as insecure/unidentified",
                masterKey.isSecure());
    }
     ...
     * {@code StandaloneMlDsa65ContentSignerBuilder} and the bcpg-fork's patched {@code
     ...
     * StandaloneMlDsa65ContentVerifierBuilderProvider} and the bcpg-fork's patched {@code
    ...
    public void testSignatureRoundTripThroughRealPgpSignatureObject() throws Exception {
        ...
        byte[] message = "the OpenKeychain standalone ML-DSA-65 round trip works".getBytes("UTF-8");
        ...
        assertEquals("signature packet must be tagged with the standalone algorithm ID",
                PublicKeyAlgorithmTags.EXPERIMENTAL_3, signature.getKeyAlgorithm());
        assertEquals("signature packet must be v6 (no v4 allowance for this algorithm)",
                6, signature.getVersion());
        assertEquals("raw signature value must be exactly the native ML-DSA-65 signature length",
                StandaloneMlDsa65.SIGNATURE_LEN, signature.getSignature().length);
        ...
        PGPContentVerifierBuilderProvider verifierBuilderProvider =
                new StandaloneMlDsa65ContentVerifierBuilderProvider();
    ...
    public void testTamperedSignatureIsRejected() throws Exception {
        ...
        byte[] signature = StandaloneMlDsa65.sign(secret, dataDigest, new SecureRandom());
        assertTrue("sanity check: the untampered signature must verify",
                StandaloneMlDsa65.verify(pub, dataDigest, signature));

        byte[] tampered = signature.clone();
        tampered[100] ^= 0x01;
        assertFalse("a bit-flip in the ML-DSA-65 signature must be rejected",
                StandaloneMlDsa65.verify(pub, dataDigest, tampered));
        ...
        assertFalse("the signature must not verify over a different dataDigest",
                StandaloneMlDsa65.verify(pub, differentDigest, signature));
    }
    ...
    public void testWrongKeyCannotVerify() throws Exception {
        ...
        StandaloneMlDsa65.KeyMaterial unrelatedKey = StandaloneMlDsa65.generateKeyPair(new SecureRandom());

        assertFalse("verifying with an unrelated key's public material must not succeed",
                StandaloneMlDsa65.verify(unrelatedKey.publicKeyBytes, dataDigest, signature));
    }
     ...
     * StandaloneMlDsa65#verify}, confirming the floor is actually enforced and not merely
     * documented.
     */
    @Test
    public void testShortDigestIsRejectedByVerify() throws Exception {
        ...
        byte[] signature = StandaloneMlDsa65.sign(secret, adequateDigest, new SecureRandom());
        ...
        assertFalse("verify() must reject a sub-256-bit dataDigest outright",
                StandaloneMlDsa65.verify(pub, shortDigest, signature));
    }
    ...
    private static byte[] extractStandaloneSecretKeyBytes(CanonicalizedSecretKey secretKey) {
        ...
        return Arrays.copyOf(rawSecretKeyData, StandaloneMlDsa65.SECRET_KEY_LEN);
    }
}
