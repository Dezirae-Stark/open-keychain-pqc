 ...
 * enforce the v6-only requirement for standalone (non-composite, closed-ecosystem) ML-DSA-87
...
public class StandaloneMlDsa87VersionEnforcementTest {

    private static final int STANDALONE_PUBLIC_KEY_LEN = 2592; // raw ML-DSA-87 public key
     ...
     * {@link PublicKeyPacket} with algorithm 103 is built directly (bypassing {@code
     * PublicKeyPacket.parseKey()} entirely), then fed straight into canonicalize().
     */
    @Test
    public void v4FramedAlgorithm103MasterKeyRejectedByCanonicalize() throws Exception {
        PublicKeyPacket v4AlgorithmOneHundredThree = new PublicKeyPacket(
                PublicKeyPacket.VERSION_4,
                PublicKeyAlgorithmTags.EXPERIMENTAL_4,
                new Date(),
                new OpaquePublicBCPGKey(new byte[STANDALONE_PUBLIC_KEY_LEN]));
        assertEquals(PublicKeyPacket.VERSION_4, v4AlgorithmOneHundredThree.getVersion());

        PGPPublicKey maliciousMasterKey =
                new PGPPublicKey(v4AlgorithmOneHundredThree, new JcaKeyFingerprintCalculator());
        ...
        assertNull("a v4-framed standalone ML-DSA-87 master key must be rejected by "
     ...
     */
    @Test
    public void v4FramedAlgorithm103KeyRejectedByRealImportPath() throws Exception {
        PublicKeyPacket v4AlgorithmOneHundredThree = new PublicKeyPacket(
                PublicKeyPacket.VERSION_4,
                PublicKeyAlgorithmTags.EXPERIMENTAL_4,
                new Date(),
        ...
        byte[] wireBytes = v4AlgorithmOneHundredThree.getEncoded();

        try {
            UncachedKeyRing.decodeFromData(wireBytes);
            fail("a v4-framed algorithm-103 (standalone ML-DSA-87) public key must be "
    ...
    /**
     * Negative-space control: a legitimately v6-framed standalone ML-DSA-87 master key (same
     ...
     * mismatch specifically, not algorithm 103 as a whole.
     */
    @Test
    public void v6FramedAlgorithm103MasterKeyNotRejectedForVersion() throws Exception {
        PublicKeyPacket v6AlgorithmOneHundredThree = new PublicKeyPacket(
        ...
        PGPPublicKey legitimateMasterKey =
                new PGPPublicKey(v6AlgorithmOneHundredThree, new JcaKeyFingerprintCalculator());
        ...
        Assert.assertFalse("a v6-framed standalone ML-DSA-87 master key must not be "
     ...
     * a v4-framed signature packet claiming algorithm 103 must be rejected during parsing
     * itself, exercising the bcpg fork's {@code SignaturePacket.parseSignature()} explicit
     * {@code EXPERIMENTAL_4} case rather than the generic {@code EXPERIMENTAL_1..
     * EXPERIMENTAL_11} fallback (which has no version check or fixed-length check at all).
     */
    @Test
    public void v4FramedAlgorithm103SignaturePacketRejectedByParser() throws Exception {
        SignaturePacket v4Sig = new SignaturePacket(
                ...
                new SignatureSubpacket[0],
                new byte[2],
                new byte[StandaloneMlDsa87.SIGNATURE_LEN],
                null);
        ...
        try {
            bcpgIn.readPacket();
            fail("a v4-framed algorithm-103 (standalone ML-DSA-87) signature packet must be "
