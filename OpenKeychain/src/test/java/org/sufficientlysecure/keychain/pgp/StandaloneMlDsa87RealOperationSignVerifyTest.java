...
/**
 * End-to-end round trip for standalone (non-composite, closed-ecosystem) ML-DSA-87
 * (OpenKeychain private-use algorithm ID 103) OpenPGP signing, exercised entirely through the
...
public class StandaloneMlDsa87RealOperationSignVerifyTest {
    ...
    public static void setUpOnce() throws Exception {
        ...
        builder.addUserId("pqc-standalone-mldsa87-real-op-test <pqc-standalone-real-op87@example.org>");
    ...
    public void testRealSignVerifyOperationRoundTrip() throws Exception {
        ...
        String plaintext = "the OpenKeychain standalone ML-DSA-87 round trip works end-to-end ☭";
        byte[] signedMessage;
        ...
        { // sign via the real PgpSignEncryptOperation, using the standalone master key
            ...
            assertTrue("real PgpSignEncryptOperation.execute() must succeed signing with the "
                    + "standalone ML-DSA-87 key: " + result.getLog(), result.success());
        ...
        { // negative control: a tampered signed message must be rejected, not silently accepted
            ...
            // the trailing signature packet's algorithm-specific (raw ML-DSA-87) data.
