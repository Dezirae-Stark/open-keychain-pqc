...
/**
 * End-to-end round trip for standalone SLH-DSA-SHAKE-128s (algorithm ID 32) OpenPGP signing,
 * exercised entirely through the same real, top-level app operation classes a caller of the app
 * actually uses -- {@link PgpSignEncryptOperation#execute} and {@link
 * PgpDecryptVerifyOperation#execute} -- with a keyring persisted in a real {@link
 * KeyWritableRepository}, following exactly the same rigor as {@code
 * CompositeMlDsa87Ed448RealOperationSignVerifyTest}.
 * <p>
 * Unlike every composite class this pattern was previously applied to, this exercises a
 * <em>standalone</em> PQC signature algorithm: no classical/ECC component, no AND-combiner --
 * just SLH-DSA's own key and signature material, used directly.
 * <p>
 * The SLH-DSA-SHAKE-128s key here is the keyring's <em>master</em> key (both certifying and
 * signing), matching the shape of the algorithm's own official test vector
 * (draft-ietf-openpgp-pqc-17's {@code v6-slhdsa-128s-sample-sk.asc}: a v6 SLH-DSA-128s Private
 * Key packet is the primary key, with a self-signed user ID). This is a structural necessity,
 * not a test-convenience choice: the draft mandates v6-only keys and signatures for this
 * algorithm too, so a v6 master key's own direct-key/user-ID self-certifications must therefore
 * also be v6 SLH-DSA signatures -- so key generation alone already exercises the v6-signature-
 * generator wiring before any message is ever signed.
 */
@RunWith(KeychainTestRunner.class)
public class SlhDsaShake128sRealOperationSignVerifyTest {
    ...
    public static void setUpOnce() throws Exception {
        ...
        builder.addUserId("pqc-slhdsa-128s-real-op-test <pqc-slhdsa-real-op@example.org>");
        ...
        assertTrue("standalone SLH-DSA-SHAKE-128s key generation must succeed (this already "
                + "exercises v6 self-certification signing): " + result.getLog(), result.success());
    ...
    /**
     * Full real-app round trip: persist the SLH-DSA keyring exactly the way the app does, sign a
     * literal-data message through {@link PgpSignEncryptOperation#execute}, verify it through
     * {@link PgpDecryptVerifyOperation#execute}, and confirm the signature is reported valid.
     * Then confirms a bit-flip in the wire bytes of the signed message is detected and rejected
     * by the same real verification call path.
     */
    @Test
    public void testRealSignVerifyOperationRoundTrip() throws Exception {
        ...
        String plaintext = "the OpenKeychain standalone SLH-DSA-SHAKE-128s round trip works end-to-end ☭";
        byte[] signedMessage;

        { // sign via the real PgpSignEncryptOperation, using the SLH-DSA master key
            ...
            assertTrue("real PgpSignEncryptOperation.execute() must succeed signing with the "
                    + "standalone SLH-DSA-SHAKE-128s key: " + result.getLog(), result.success());
        ...
        { // verify via the real PgpDecryptVerifyOperation
            ...
            assertTrue("real PgpDecryptVerifyOperation.execute() must succeed verifying the "
                    + "SLH-DSA-signed message: " + result.getLog(), result.success());
        ...
        { // negative control: a tampered signed message must be rejected, not silently accepted
            ...
            // the trailing signature packet's algorithm-specific (raw SLH-DSA) data.
            int flipOffset = tampered.length - 200;
            tampered[flipOffset] ^= 0x01;

            DecryptVerifyResult result = runVerify(tampered, null);

            Assert.assertEquals("a bit-flipped SLH-DSA signature must be reported invalid, "
